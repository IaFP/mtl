...
{-# LANGUAGE PartialTypeConstructors, TypeOperators, DefaultSignatures, UndecidableSuperClasses #-}
#endif
...
instance (Monoid w, Monad m
         ...
         , Total m
#endif
         ) => MonadWriter w (Lazy.WriterT w m) where
    writer = Lazy.writer
    tell   = Lazy.tell
    listen = Lazy.listen
    pass   = Lazy.pass
...
instance (Monoid w, Monad m
         ...
         , Total m
#endif
         ) => MonadWriter w (Strict.WriterT w m) where
...
instance (Monoid w, Monad m
         ...
         , Total m
#endif
         ) => MonadWriter w (LazyRWS.RWST r w s m) where
...
instance (Monoid w, Monad m
         ...
         , Total m
#endif
         ) => MonadWriter w (StrictRWS.RWST r w s m) where
...
instance (Error e, MonadWriter w m
#if MIN_VERSION_base(4,14,0)
         , Total m
#endif
         ) => MonadWriter w (ErrorT e m) where
    writer = lift . writer
    tell   = lift . tell
...
instance (MonadWriter w m
         ...
         , Total m
#endif
         ) => MonadWriter w (ExceptT e m) where
    writer = lift . writer
    tell   = lift . tell
...
instance (MonadWriter w m
         ...
         , Total m
#endif
         ) => MonadWriter w (IdentityT m) where
    writer = lift . writer
    tell   = lift . tell
...
instance (MonadWriter w m
         ...
         , Total m
#endif
         ) => MonadWriter w (ReaderT r m) where
    writer = lift . writer
    tell   = lift . tell
...
instance (MonadWriter w m
         ...
         , Total m
#endif
         ) => MonadWriter w (Lazy.StateT s m) where
    writer = lift . writer
    tell   = lift . tell
...
instance (MonadWriter w m
         ...
         , Total m
#endif
         ) => MonadWriter w (Strict.StateT s m) where
    writer = lift . writer
    tell   = lift . tell

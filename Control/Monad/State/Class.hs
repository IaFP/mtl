...
{-# LANGUAGE PartialTypeConstructors, TypeOperators, UndecidableInstances, DefaultSignatures, UndecidableSuperClasses #-}
#endif
...
instance (Monad m
        ...
        , Total m
#endif
         ) => MonadState s (Lazy.StateT s m) where
    get = Lazy.get
    put = Lazy.put
    state = Lazy.state
...
instance (Monad m
        ...
        , Total m
#endif
         ) => MonadState s (Strict.StateT s m) where
    get = Strict.get
    put = Strict.put
    state = Strict.state
...
instance (Monad m, Monoid w
        ...
        , Total m
#endif
         ) => MonadState s (LazyRWS.RWST r w s m) where
    get = LazyRWS.get
    put = LazyRWS.put
    state = LazyRWS.state
...
instance (Monad m, Monoid w
        ...
        , Total m
#endif
         ) => MonadState s (StrictRWS.RWST r w s m) where
    get = StrictRWS.get
    put = StrictRWS.put
...
instance (Monoid w, MonadState s m
#if MIN_VERSION_base(4,14,0)
        , Total m
#endif
         ) => MonadState s (Lazy.WriterT w m) where
    get = lift get
    put = lift . put
    state = lift . state
...
instance (Monoid w, MonadState s m
        ...
        , Total m
#endif
         ) => MonadState s (Strict.WriterT w m) where
    get = lift get
    put = lift . put
    state = lift . state
